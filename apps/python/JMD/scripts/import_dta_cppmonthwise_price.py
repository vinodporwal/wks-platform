"""
Import missing DTA CPPMonthWisePrice rows from DTA_JMD.ods
============================================================
For every DTA NormsHeader record, checks that a CPPMonthWisePrice row exists for
FY 2026-27. Missing rows are inserted with monthly prices read from DTA_JMD.ods.
Empty ODS cells are treated as 0.

Usage:
    python scripts/import_dta_cppmonthwise_price.py              # dry run
    python scripts/import_dta_cppmonthwise_price.py --execute    # insert
"""
...
def _read_ods_data():
    """Read all data rows from DTA_JMD.ods with monthly prices."""
...
def main():
    parser = argparse.ArgumentParser(description="Import missing DTA CPPMonthWisePrice rows from ODS")
    ...
    try:
        ...
        sql_file = os.path.join(OUTPUT_DIR, f"import_dta_cppmonthwise_price_{timestamp}.sql")
        ...
        manifest_file = os.path.join(MANIFEST_DIR, f"import_dta_cppmonthwise_price_{timestamp}.json")
