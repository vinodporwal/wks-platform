"""
Import missing DTA NormsMonthDetail rows from DTA_JMD.ods
===========================================================
For every DTA NormsHeader record, checks that all 12 months (April 2026 - March 2027)
have a NormsMonthDetail row. Missing months are inserted with values read from the
DTA_JMD.ods file. Empty ODS cells are treated as 0.

Usage:
    python scripts/import_dta_norms_month_detail.py              # dry run
    python scripts/import_dta_norms_month_detail.py --execute    # insert
"""
...
# CONFIG
# ---------------------------------------------------------------------------
ODS_PATH = os.path.join(JMD_DIR, "..", "files", "DTA_JMD.ods")
...
# DTA_JMD.ods month block: 4 columns per month (Norms, Qty, Amount, Price)
FIRST_MONTH_COL = 11
MONTH_BLOCK_WIDTH = 4
NUM_MONTHS = 12
...
def _read_ods_data():
    """Read all data rows from DTA_JMD.ods with monthly values."""
...
def _build_plant_map(ods_rows, db_plants):
    """Map each ODS Utility Plant name to a DB Plant.Id."""
...
def _match_rows(ods_rows, norms_headers, plant_map):
    ...
    # Build ODS index by (plant_id, utility, account, material)
    ods_index = {}
    for ods in ods_rows:
    ...
    for nh in norms_headers:
        ...
        if len(candidates) == 1:
            ...
            # Try to match by issuing plant
            nh_issuing = _normalize(nh["issuing_plant"])
            for cand in candidates:
                cand_issuing = _normalize(cand["issuing_plant"])
                if cand_issuing == nh_issuing:
                    selected = cand
                    break
                # DB issuing might be full plant name; ODS might be short code
                if cand_issuing in nh_issuing or nh_issuing in cand_issuing:
                    selected = cand
                    break
            if not selected:
                selected = candidates[0]
        ...
        # Mark used by a composite key
        used_ods_keys.add((
            _normalize(selected["utility_plant"]),
            _normalize(selected["utility"]),
            _normalize(selected["account"]),
            _normalize(selected["material"]),
            _normalize(selected["issuing_plant"]),
        ))

    unmatched_ods = []
    for ods in ods_rows:
        k = (
            _normalize(ods["utility_plant"]),
            _normalize(ods["utility"]),
            _normalize(ods["account"]),
            _normalize(ods["material"]),
            _normalize(ods["issuing_plant"]),
        )
        if k not in used_ods_keys:
...
def _build_insert_params(record_id, nh_id, fym_id, values):
    return [
        ...
        values["amount"],
        values["price"],
        0.0,               # QTY (not present in DTA ODS)
        None,              # GenerationUOM
        None,              # ScenarioType
        None,              # DisplayOrder
        None,              # Remarks
...
def main():
    parser = argparse.ArgumentParser(description="Import missing DTA NormsMonthDetail rows from ODS")
    ...
    cur.execute("SELECT Id, Name, SourceName, IsActive FROM Plants WHERE IsActive = 1")
    db_plants = []
    for row in cur.fetchall():
        db_plants.append({
            "id": str(row[0]),
    ...
    print("Loading DTA NormsHeader rows...")
    ...
    print(f"  Existing NormsMonthDetail rows for matched DTA headers: {len(existing)}")
    ...
    try:
        ...
        sql_file = os.path.join(OUTPUT_DIR, f"import_dta_norms_month_detail_{timestamp}.sql")
        ...
        manifest_file = os.path.join(MANIFEST_DIR, f"import_dta_norms_month_detail_{timestamp}.json")
