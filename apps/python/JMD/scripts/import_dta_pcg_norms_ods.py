"""
JMD DTA-PCG Norms Import Script (ODS Source)
=============================================
Imports Norm/Quantity/Amount/Price data for JMD DTA-PCG plants from the ODS file
"DTA-PCG_JMD.ods" (OpenDocument Spreadsheet) into:
    Plants -> NormParameters -> NormsHeader -> NormsMonthDetail
                                        -> CPPNorms
                                        -> CPPMonthWisePrice

Usage:
    python scripts/import_dta_pcg_norms_ods.py                 # dry run (default)
    python scripts/import_dta_pcg_norms_ods.py --execute        # actually insert
...
# CONFIG
# ============================================================
ODS_PATH = os.path.join(JMD_DIR, "..", "files", "DTA-PCG_JMD.ods")

# DTA-PCG has only one plant in the ODS.
ALLOWED_PLANTS = [
    "RIL-JW Plant-DTA PCG",
]
...
SIBLING_PLANT_NAME_FOR_NEW_ROWS = "RIL-JW Plant-DTA PCG"

# No new Plants rows needed - the DTA-PCG plant already exists in the DB.
NEW_PLANTS_TO_CREATE = {}

# DTA-PCG-CPP parent plant UUID - used as NormType reference.
...
NORM_TYPE_REFERENCE_PLANT_ID = "F6D82E68-C3B6-494F-9905-48F19DC611E3"

# DTA-PCG ODS layout (same as SEZ-PCG ODS):
...
# Hardcoded FY since DTA-PCG ODS has no FY label
FY_START_YEAR = 2026
...
SQL_OUTPUT_FILE = os.path.join(OUTPUT_DIR, f"jmd_dta_pcg_norms_ods_import_{datetime.now():%Y%m%d_%H%M%S}.sql")
MANIFEST_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manifests")
MANIFEST_FILE = os.path.join(MANIFEST_DIR, f"jmd_dta_pcg_norms_ods_import_{datetime.now():%Y%m%d_%H%M%S}.json")
DRY_RUN = True

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("jmd_dta_pcg_norms_ods_import")
...
def load_ods_data(path: str = ODS_PATH) -> pd.DataFrame:
    ...
    # DTA-PCG ODS has no FY label - use hardcoded FY_START_YEAR
    ...
    # Extract month periods from MONTH_NAME_ROW_IDX (row 0 for DTA-PCG)
    month_periods = []
    ...
    logger.info(f"Rows after filtering to allowed JMD DTA-PCG plants: {len(data)}")
    ...
    for _, row in data.iterrows():
        ...
        for month_idx, (month_num, cal_year) in enumerate(month_periods):
            ...
            # Even when all four month values are blank, we still insert a
            # NormsMonthDetail row with 0 values so that every header has
            # exactly 12 month rows (170 headers x 12 months = 2040 rows).
            if _all_blank([norms, quantity, amount, price]):
                ...
                norms_val = _to_float(norms) or 0.0
                quantity_val = _to_float(quantity) or 0.0
                amount_val = _to_float(amount) or 0.0
                price_val = _to_float(price) or 0.0
            rec = dict(identity)
            rec.update({
                "month_num": month_num,
...
# ============================================================
# STEP 4: Fetch JMD DTA-PCG plants
# ============================================================
def fetch_jmd_plants(conn) -> tuple:
    ...
    if missing:
        ...
        logger.info(f"All required JMD DTA-PCG plants found: {plant_map}")
...
def ensure_missing_plants_exist(conn, dry_run: bool) -> dict:
    ...
    For DTA-PCG, NEW_PLANTS_TO_CREATE is empty (plant already exists), so
    this is a no-op that just returns empty results.
    """
    if not NEW_PLANTS_TO_CREATE:
        logger.info("No new Plants rows needed - all DTA-PCG plants already exist.")
        return {"created": [], "already_existed": list(ALLOWED_PLANTS), "created_ids": []}
    raise NotImplementedError("Plant creation not configured for DTA-PCG.")
...
def fetch_norm_type_reference(conn) -> dict:
    """Build {utility_name_upper: NormType_FK_Id} from DTA-PCG-CPP parent's NormParameters."""
    cur = conn.cursor()
    cur.execute(
    ...
    logger.info(f"Built NormType reference with {len(reference)} names from DTA-PCG-CPP.")
    return reference
...
def generate_sql_file(
    ...
    lines = []
    lines.append("-- ============================================================")
    lines.append("-- JMD DTA-PCG Norms Import (ODS) - Generated SQL")
    ...
    for p in price_records:
        ...
        extra_vals = (
            ...
            f"{_sql_str(p['aop_year'])}, N'Imported from DTA-PCG_JMD.ods', "
            f"N'DTA-PCG_JMD.ods', GETDATE(), GETDATE(), N'SYSTEM', N'Price'"
        )
        lines.append(
...
def print_validation_report(
    ...
    print("\n" + "=" * 70)
    print("JMD DTA-PCG NORMS IMPORT (ODS) - VALIDATION REPORT")
    print("=" * 70)
    print(f"Total ODS rows (month-level, JMD DTA-PCG plants only): {total_rows}")
        ...
        for p in price_records:
            ...
            cur.execute(
                ...
                [p["id"]] + month_vals + [
                    ...
                    "Imported from DTA-PCG_JMD.ods", "DTA-PCG_JMD.ods",
                    "SYSTEM", "Price",
                ],
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Import JMD DTA-PCG norms from ODS into NormsHeader + transactional tables"
    )
    parser.add_argument(
