...
def _log_utility_demand_rollup(process: dict, fixed: dict, month: int, year: int) -> None:
    ...
    logger.info("  [DEMAND] %-24s  %12s  %12s  %12s  %-8s", "Utility", "Process", "Fixed", "Total", "Unit")
    logger.info("  [DEMAND] %s  %s  %s  %s  %s", "-" * 24, "-" * 12, "-" * 12, "-" * 12, "-" * 8)
    ...
    for label, process_key, fixed_key, unit, convert_process_to_mwh in utility_rows:
        ...
        display_unit = "MWh" if convert_process_to_mwh else unit
        logger.info(
            "  [DEMAND] %-24s  %12.2f  %12.2f  %12.2f  %-8s",
            label,
            display_process,
            fixed_val,
            total_val,
