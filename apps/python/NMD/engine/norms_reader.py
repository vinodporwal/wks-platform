...
the data into a consumption matrix that the U4U iteration loop can use to
...
class NMDNormsReader:
    ...
    def _build_indexes(self):
        ...
        for row in self._rows:
            ...
            # Track all generation utilities (all accounts with utility names)
            # Any utility with norms is considered a generation utility
            # Pass-through utilities are those with demand but no norms
            # Skip Power_Dis: it is a distribution/sink utility, not a producer;
            # including it creates a circular U4U dependency where total power demand
            # drives its own auxiliary consumption and prevents convergence.
            if utility.upper() != "POWER_DIS":
                self._generation_utilities.add(utility)

            # Track U4U materials (all accounts)
            # These are consumed by generation utilities but not generated by NMD plant
            self._u4u_materials.add(material)

            # Build consumption matrix entry
            # For POWERGEN, key by plant_name to keep each asset separate
            if utility.upper() == "POWERGEN" and plant:
                matrix_key = plant
            else:
                matrix_key = utility

            if matrix_key not in self._consumption_matrix:
                self._consumption_matrix[matrix_key] = {
                    "plant_name": plant,
                    "utility": utility,
                    "consumptions": [],
                }

            self._consumption_matrix[matrix_key]["consumptions"].append({
                "account": account,
                "material": material,
    ...
    def get_consumption_norms_flat(self) -> Dict[str, Dict[str, float]]:
        ...
        Includes ALL account entries (not just Utilities/Catalyst & Chemical/Raw Material).
        """
    ...
    def get_u4u_materials(self) -> Set[str]:
        """Return the set of all U4U material names (Utilities, Catalyst & Chemical, and Raw Material accounts)."""
