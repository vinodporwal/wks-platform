"""
JMD SEZ Norms Import Rollback Script
=====================================
Undoes a completed run of import_jmd_sez_norms_ods.py --execute, using the manifest
...
Usage:
    python scripts/rollback_jmd_sez_norms.py <manifest.json>              # dry run (default)
    python scripts/rollback_jmd_sez_norms.py <manifest.json> --execute    # actually delete
"""
...
logger = logging.getLogger("rollback_jmd_sez_norms")
...
def print_rollback_report(conn, manifest: dict):
    print("\n" + "=" * 70)
    print("JMD SEZ NORMS ROLLBACK - DRY RUN REPORT")
    print("=" * 70)
...
def main():
    parser = argparse.ArgumentParser(
        description="Roll back a completed JMD SEZ norms import using its manifest file."
    )
    parser.add_argument("manifest", help="Path to the jmd_sez_norms_ods_import_*.json manifest file.")
    parser.add_argument("--execute", action="store_true",
                         help="Actually delete records. Without this flag, runs as a dry run.")
    args = parser.parse_args()
    dry_run = not args.execute

    logger.info(f"Starting JMD SEZ norms rollback. DRY_RUN={dry_run}")
