"""
JMD SEZ Norms Import Script (ODS Source)
=========================================
Imports Norm/Quantity/Amount/Price data for JMD SEZ plants from the ODS file
"SEZ_JMD.ods" (OpenDocument Spreadsheet) into:
    Plants -> NormParameters -> NormsHeader -> NormsMonthDetail
                                        -> CPPNorms

Usage:
    python scripts/import_jmd_sez_norms_ods.py                 # dry run (default)
    python scripts/import_jmd_sez_norms_ods.py --execute        # actually insert
...
Phase 2 (real run, DRY_RUN=False, requires --execute):
    ...
    - On commit, writes a manifest of every inserted Id to scripts/manifests/,
      so this run can be undone later with rollback_jmd_sez_norms.py.
"""
...
# All 10 SEZ plants present in the ODS. Verified to already exist in the
# Plants table (each with SourceName = SEZ-CPP parent Id), so no new Plants
# rows need to be created.
ALLOWED_PLANTS = [
    "JMD - SEZ Distribution (Power)",
    "JMD - SEZ GT Power plant 1",
    "JMD - SEZ GT Power plant 2",
    "JMD - SEZ GT Power plant 3",
    "JMD - SEZ GT Power plant 4",
    "JMD - SEZ GT Power plant 5",
    "JMD - SEZ GT Power plant 6",
    "JMD - SEZ STG Power plant 1",
    "JMD - SEZ STG Power plant 2",
    "JMD - SEZ Utility Plant",
]
...
SIBLING_PLANT_NAME_FOR_NEW_ROWS = "JMD - SEZ Utility Plant"

# No new Plants rows needed - all 10 SEZ plants already exist in the DB.
NEW_PLANTS_TO_CREATE = {}

# SEZ-CPP plant UUID (from plant_mapper.py) - used as NormType reference.
...
NORM_TYPE_REFERENCE_PLANT_ID = "2DFEE33F-4CFD-4887-B9DD-53388AA95271"

# SEZ ODS layout (identical to DTA ODS):
...
SQL_OUTPUT_FILE = os.path.join(OUTPUT_DIR, f"jmd_sez_norms_ods_import_{datetime.now():%Y%m%d_%H%M%S}.sql")
MANIFEST_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manifests")
MANIFEST_FILE = os.path.join(MANIFEST_DIR, f"jmd_sez_norms_ods_import_{datetime.now():%Y%m%d_%H%M%S}.json")
DRY_RUN = True

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("jmd_sez_norms_ods_import")
...
def load_ods_data(path: str = ODS_PATH) -> pd.DataFrame:
    ...
    logger.info(f"Rows after filtering to allowed JMD SEZ plants: {len(data)}")
...
def fetch_jmd_plants(conn) -> tuple:
    ...
    else: logger.info(f"All required JMD SEZ plants found: {plant_map}")
...
def ensure_missing_plants_exist(conn, dry_run: bool) -> dict:
    ...
    For SEZ, NEW_PLANTS_TO_CREATE is empty (all 10 plants already exist), so
    this is a no-op that just returns empty results. Kept for structural parity
    with the DTA/C2 import scripts.
    """
    if not NEW_PLANTS_TO_CREATE:
        logger.info("No new Plants rows needed - all SEZ plants already exist.")
        return {"created": [], "already_existed": list(ALLOWED_PLANTS), "created_ids": []}

    cur = conn.cursor()
    # Fetch SEZ-CPP parent plant to use as parent (SourceName)
    cur.execute("SELECT Id, Site_FK_Id, Vertical_FK_Id, DisplayOrder, IsActive FROM Plants WHERE Name = 'SEZ-CPP'")
    cpp_parent = cur.fetchone()
    if not cpp_parent:
        raise RuntimeError("Parent plant 'SEZ-CPP' not found.")
...
def fetch_norm_type_reference(conn) -> dict:
    """Build {utility_name_upper: NormType_FK_Id} from SEZ-CPP parent's NormParameters."""
    cur = conn.cursor()
    ...
    logger.info(f"Built NormType reference with {len(reference)} names from SEZ-CPP.")
    return reference
...
def generate_sql_file(headers, month_details, cpp_records, check_queries: list, out_path: str = SQL_OUTPUT_FILE, new_norm_parameters: list = None):
    ...
    lines = []
    lines.append("-- ============================================================")
    lines.append("-- JMD SEZ Norms Import (ODS) - Generated SQL")
...
def print_validation_report(df, plant_ids, headers, month_details, cpp_records, column_problems, new_norm_parameters=None):
    ...
    print("\n" + "=" * 70)
    print("JMD SEZ NORMS IMPORT (ODS) - VALIDATION REPORT")
    print("=" * 70)
    print(f"Total ODS rows (month-level, JMD SEZ plants only): {total_rows}")
...
def main():
    global DRY_RUN
    parser = argparse.ArgumentParser(description="Import JMD SEZ norms from ODS file")
    parser.add_argument("--execute", action="store_true", help="Actually insert records (default: dry run)")
    args = parser.parse_args()
    DRY_RUN = not args.execute

    logger.info(f"{'=' * 60}")
    logger.info(f"JMD SEZ Norms Import (ODS) - {'DRY RUN' if DRY_RUN else 'EXECUTE MODE'}")
    logger.info(f"{'=' * 60}")
    ...
    if df.empty:
        logger.error("No JMD SEZ rows found in ODS after filtering. Aborting.")
        return
    ...
    try:
        ...
        if DRY_RUN:
            ...
            logger.error("Not all required JMD SEZ plants exist in the Plants table. Aborting.")
            return
        ...
        if not DRY_RUN:
            ...
            logger.info(f"To undo this import: python scripts/rollback_jmd_sez_norms.py {MANIFEST_FILE} --execute")
        else:
