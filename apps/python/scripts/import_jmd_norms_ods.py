"""
JMD C2 Norms Import Script (ODS Source)
========================================
Imports Norm/Quantity/Amount/Price data for JMD C2 plants from the ODS file
"C2_JMD.ods" (OpenDocument Spreadsheet) into:
    Plants -> NormParameters -> NormsHeader -> NormsMonthDetail
                                        -> CPPNorms

Usage:
    python scripts/import_jmd_norms_ods.py                 # dry run (default)
    python scripts/import_jmd_norms_ods.py --execute        # actually insert
"""
...
ALLOWED_PLANTS = [
    "JMD - C2 Utility Plant", "JMD - C2-GTG 1", "JMD - C2-GTG 2",
    "JMD - C2-STG 1", "JMD - DTA-C2 Power & UTILITY",
]
...
SQL_OUTPUT_FILE = os.path.join(OUTPUT_DIR, f"jmd_norms_ods_import_{datetime.now():%Y%m%d_%H%M%S}.sql")
MANIFEST_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), "manifests")
MANIFEST_FILE = os.path.join(MANIFEST_DIR, f"jmd_norms_ods_import_{datetime.now():%Y%m%d_%H%M%S}.json")
DRY_RUN = True

logging.basicConfig(level=logging.INFO, format="%(asctime)s [%(levelname)s] %(message)s")
logger = logging.getLogger("jmd_norms_ods_import")
...
def validate_ods_columns(raw_header_row) -> list:
    problems = []
    expected_identity = {
        COL_UTILITY_PLANT: "Utility Plant", COL_UTILITY_PLANT_ID: "Utility Plant ID",
        COL_UTILITY: "Utility", COL_UTILITY_ID: "Utility ID", COL_UTILITY_UOM: "Utility UOM",
        COL_ACCOUNT: "Account", COL_MATERIAL: "Material", COL_MATERIAL_ID: "Material ID",
        COL_MATERIAL_UOM: "Material UOM", COL_ISSUING_PLANT: "Issuing Plant",
    ...
    for month_idx in range(NUM_MONTHS):
        base_col = FIRST_MONTH_COL + month_idx * MONTH_BLOCK_WIDTH
        for offset, metric in enumerate(["Norms", "Quantity", "Amount", "Price", "Generation Qty", "Rate Per Unit"]):
...
def fetch_jmd_plants(conn) -> tuple:
    ...
    else: logger.info(f"All required JMD C2 plants found: {plant_map}")
    return plant_map, plant_code_map

def ensure_missing_plants_exist(conn, dry_run: bool) -> dict:
    cur = conn.cursor()
    # Fetch C2-CPP plant to use as parent (SourceName)
    cur.execute("SELECT Id, Site_FK_Id, Vertical_FK_Id, DisplayOrder, IsActive FROM Plants WHERE Name = 'C2-CPP'")
    cpp_parent = cur.fetchone()
    if not cpp_parent:
        raise RuntimeError("Parent plant 'C2-CPP' not found.")
...
def generate_sql_file(headers, month_details, cpp_records, check_queries: list, out_path: str = SQL_OUTPUT_FILE, new_norm_parameters: list = None):
    ...
    lines = []
    lines.append("-- ============================================================")
    lines.append("-- JMD C2 Norms Import (ODS) - Generated SQL")
...
# ============================================================
def insert_records(conn, new_norm_parameters: list, headers: list, month_details: list, cpp_records: list) -> dict:
    """Insert all new records in a single transaction. Returns manifest of inserted IDs."""
    manifest = {
        "norm_parameters": [],
        "norms_headers": [],
        "norms_month_details": [],
    ...
    cur = conn.cursor()
    try:
        # Insert NormParameters
        for p in new_norm_parameters:
            cur.execute(
                "INSERT INTO NormParameters (Id, Name, DisplayName, UOM, Type, NormParameterType_FK_Id, Plant_FK_Id, NormType_FK_Id, IsHistorical, DisplayOrder, IsEditable, IsVisible, CalculationType, SAPMaterialCode) VALUES (CAST(? AS uniqueidentifier), ?, ?, ?, ?, ?, CAST(? AS uniqueidentifier), ?, ?, ?, ?, ?, ?, ?)",
...
def main():
    global DRY_RUN
    parser = argparse.ArgumentParser(description="Import JMD C2 norms from ODS file")
    parser.add_argument("--execute", action="store_true", help="Actually insert records (default: dry run)")
    args = parser.parse_args()
    DRY_RUN = not args.execute

    logger.info(f"{'=' * 60}")
    logger.info(f"JMD C2 Norms Import (ODS) - {'DRY RUN' if DRY_RUN else 'EXECUTE MODE'}")
    logger.info(f"{'=' * 60}")

    # Step 1: Load ODS data
    raw = pd.read_excel(ODS_PATH, engine="odf", header=None)
    validate_ods_columns(raw.iloc[HEADER_ROW_IDX])
    ...
    # Step 9: Insert records (if not dry run)
    if not DRY_RUN:
        manifest = insert_records(conn, new_norm_parameters, headers, month_details, cpp_records)
