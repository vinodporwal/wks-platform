...
public class FinalNormsServiceImpl implements FinalNormsService {
	...
	public AOPMessageVM getFinalNorms(String year, String plantId, String mode, String method) {
		...
		boolean crackerC2AndHmd = vertical.getName().equalsIgnoreCase("Cracker")
				&& (site.getName().equalsIgnoreCase("C2")
						|| site.getName().equalsIgnoreCase("HMD")
						|| site.getName().equalsIgnoreCase("DMD"));
		try {
			...
			for (Object[] row : results) {
				...
				if (crackerC2AndHmd && row.length > 27 && row[27] != null) {
					try {
						finalNormsDTO.setWtAverage(Double.parseDouble(row[27].toString()));
					} catch (NumberFormatException e) {
						finalNormsDTO.setWtAverage(0.0);
					}
				}
