...
public interface ShutDownPlanRepository extends JpaRepository<PlantMaintenanceTransaction, UUID>{
	...
	@Query(value = "SELECT " +
            ...
            "pmt.MaintenanceText, " +
            "pm.Id, " +
            "np.Id AS NormParameter_FK_Id, pm.Remarks, NPT.DisplayOrder, pm.Line_FK_Id, pm.ShutdownRate " +
	...
	List<Object[]> findMaintenanceDetailsByPlantIdAndType( 
        ...
        @Query(value = "SELECT " +
            ...
            "pmt.MaintenanceText, " +
            "pm.Id, " +
            "np.Id AS NormParameter_FK_Id, pm.Remarks, NPT.DisplayOrder, pm.Line_FK_Id, pm.ShutdownRate, pm.Rate " +
            "FROM PlantMaintenanceTransaction pm " +
            "JOIN PlantMaintenance pmt ON pm.PlantMaintenance_FK_Id = pmt.Id " +
            "JOIN MaintenanceTypes mt ON pmt.MaintenanceType_FK_Id = mt.Id " +
            "LEFT JOIN NormParameters np ON pm.NormParameter_FK_Id = np.Id " +
			...
			"and pm.AuditYear = :year order by pm.MaintStartDateTime",
            nativeQuery = true)
	List<Object[]> findMaintenanceDetailsWithValueByPlantIdAndType( 
        @Param("maintenanceTypeName") String maintenanceTypeName, @Param("plantId") String plantId,  @Param("year") String year);
        ...
        @Query(value = "SELECT pm.Discription, pm.DurationInMins, pm.MaintForMonth FROM PlantMaintenanceTransaction pm WHERE pm.Id = :id", nativeQuery = true)
        List<Object[]> findPlantMaintenanceById(@Param("id") UUID id);
	...
	@Query(value = "SELECT " +
            ...
            "WHERE mt.Name = :maintenanceTypeName " +
            "and pmt.Plant_FK_Id = :plantId " +
            "and pm.AuditYear = :year and pm.Discription = :discription " +
            "and pm.Line_FK_Id = :lineId order by pm.CreatedOn desc",
            nativeQuery = true)
	List<Object[]> findDiscriptionByPlantIdAndTypeAndLine(
        @Param("maintenanceTypeName") String maintenanceTypeName, @Param("plantId") String plantId,
        @Param("year") String year, @Param("discription") String discription,
