...
public interface BudgetMaintenanceRepository extends JpaRepository<BudgetMaintenance, UUID>{
	...
	@NativeQuery("SELECT * FROM BudgetMaintenance bm WHERE bm.PlantId = :plantId AND bm.AOPYear = :aopYear AND bm.budgetCategory = :budgetCategory order by Sequence")
	...
	@NativeQuery("SELECT * FROM BudgetMaintenance bm WHERE  bm.AOPYear = :aopYear AND bm.budgetCategory = :budgetCategory order by Sequence")
