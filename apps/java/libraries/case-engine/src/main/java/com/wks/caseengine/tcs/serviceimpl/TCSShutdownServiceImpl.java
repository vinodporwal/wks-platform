...
import com.wks.caseengine.tcs.repository.TCSShutdownRepository;
...
if(plantId != null) {
    ...
    public AOPMessageVM carryForwardTCSShutdown(String plantId, String year) {
        try {
            ...
            String procedureName = verticalName + "_" + siteName + "_CarryForwardTcsShutdown";
    ...
    public byte[] exportTCSShutdown(
        ...
        try {
            ...
            List<TCSShutdownDTO> dtoList = (List<TCSShutdownDTO>) dataMap.get("results");
            
            System.out.println("Data list: " + dtoList);

            Workbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("TCS Shutdown");
            ...
            String[] headers = {"Particulars", "Duration In Days", "Start Date", "End Date", "Purpose", "Id"};
            ...
            for (TCSShutdownDTO dto : dtoList) {
                ...
                // Start Date - store as date value, not string
                ...
                // End Date - store as date value, not string
    ...
    public AOPMessageVM importExcel(
        ...
        try {
            List<TCSShutdownDTO> data = readTCSShutdown(file.getInputStream());
            ...
            List<TCSShutdownDTO> validRecords = new ArrayList<>();
            List<TCSShutdownDTO> failedRecords = new ArrayList<>();
            
            for (TCSShutdownDTO dto : data) {
            ...
            if (!validRecords.isEmpty()) {
                try {
                    ...
                    for (TCSShutdownDTO dto : validRecords) {
    ...
    private List<TCSShutdownDTO> readTCSShutdown(InputStream inputStream) {
        List<TCSShutdownDTO> dataList = new ArrayList<>();
        ...
        try (Workbook workbook = new XSSFWorkbook(inputStream)) {
            ...
            while (rowIterator.hasNext()) {
                ...
                try {
                    ...
                    // Note: If Id is null/empty, it will be treated as new record (insert)
                    ...
                    if (dto.getSaveStatus() == null) {
                        ...
                        } else if (dto.getDurationInDays() == null || dto.getDurationInDays() <= 0) {
                            dto.setSaveStatus("Failed");
                            System.out.println("Duration In Days is required");
                            dto.setErrDescription("Duration In Days must be greater than 0");
                        } else if (dto.getPurpose() == null || dto.getPurpose().isEmpty()) {
                            System.out.println("Purpose is required");
                            dto.setSaveStatus("Failed");
    ...
    private byte[] exportWithStatus(List<TCSShutdownDTO> dtoList) {
        try {
            Workbook workbook = new XSSFWorkbook();
            Sheet sheet = workbook.createSheet("TCS Shutdown");
            ...
            Row headerRow = sheet.createRow(currentRow++);
            String[] headers = {"Particulars", "Duration In Days", "Start Date", "End Date", "Purpose", "Id", "Status", "Error Description"};
            ...
            // Auto-size columns
            for (int i = 0; i < headers.length; i++) {
                if (i == 4 || i == 7) { // Purpose and Error Description columns
                    sheet.setColumnWidth(i, 8000);
                } else {
                    sheet.autoSizeColumn(i);
                }
            ...
            sheet.setColumnHidden(5, true); // Hide Id column
    ...
    private Date getDateCellValue(Cell cell, DateFormat dateFormatter) {
        System.out.println("getDataCellValue called");
        if (cell == null || cell.getCellType() == CellType.BLANK) {
            return null;
        }
