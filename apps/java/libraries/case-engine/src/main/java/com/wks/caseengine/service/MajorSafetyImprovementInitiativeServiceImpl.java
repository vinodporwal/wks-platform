...
public class MajorSafetyImprovementInitiativeServiceImpl implements MajorSafetyImprovementInitiativeService {
    ...
    public AOPMessageVM getMajorSafetyImprovementInitiative(String aopYear, String siteId) {
        ...
        try {
            ...
            for (Object[] row : results) {
                ...
                dto.setCategory(row.length > 6 && row[6] != null ? row[6].toString() : "");
                dto.setOutcome(row.length > 7 && row[7] != null ? row[7].toString() : "");
                dto.setRecommendation(row.length > 8 && row[8] != null ? row[8].toString() : "");
                dto.setTargetDate(row.length > 9 && row[9] != null ? parseDate(row[9]) : null);
                dto.setResponsibility(row.length > 10 && row[10] != null ? row[10].toString() : "");
                dto.setRemark(row.length > 10 && row[10] != null ? row[10].toString() : "");
                dto.setSiteId(row.length > 11 && row[11] != null ? row[11].toString() : "");
                dto.setSiteFkId(row.length > 11 && row[11] != null ? row[11].toString() : "");
                dto.setAopYear(row.length > 12 && row[12] != null ? row[12].toString() : "");
                dto.setModifiedBy(row.length > 13 && row[13] != null ? row[13].toString() : "");
                dto.setUpdatedBy(row.length > 13 && row[13] != null ? row[13].toString() : "");
                dto.setModifiedOn(row.length > 14 && row[14] != null ? parseDate(row[14]) : null);
                dto.setUpdatedDateTime(row.length > 14 && row[14] != null ? parseDate(row[14]) : null);
                list.add(dto);
            }
    ...
    public AOPMessageVM updateMajorSafetyImprovementInitiative(List<MajorSafetyImprovementInitiativeDTO> dtoList) {
        ...
        try {
            for (MajorSafetyImprovementInitiativeDTO dto : dtoList) {
                MajorSafetyImprovementInitiative entity = null;
                ...
                if (entity == null) {
                    entity = new MajorSafetyImprovementInitiative();
                }
                ...
                entity.setResponsibility(dto.getResponsibility() != null ? dto.getResponsibility() : dto.getRemark());
    ...
    @Override
    public AOPMessageVM deleteMajorSafetyImprovementInitiative(String id) {
        AOPMessageVM aopMessageVM = new AOPMessageVM();
        ...
        try {
            ...
            majorSafetyImprovementInitiativeRepository.deleteById(uuid);
            aopMessageVM.setCode(200);
            ...
            throw new RuntimeException("Failed to delete major safety improvement initiative", ex);
        }
    }

    @Override
    public AOPMessageVM getPlantDropdownForSiteAOPReport(String siteId) {
        AOPMessageVM aopMessageVM = new AOPMessageVM();
        try {
            UUID.fromString(siteId); // validate UUID format

            String sql = "EXEC Sp_GetPlantDropdownForSiteAOPReport @SiteId = :siteId";
            Query query = entityManager.createNativeQuery(sql);
            query.setParameter("siteId", UUID.fromString(siteId));
            ...
            for (Object[] row : results) {
                ...
                map.put("id", row.length > 0 && row[0] != null ? row[0].toString() : "");
                map.put("plantName", row.length > 1 && row[1] != null ? row[1].toString() : "");
                map.put("plantDisplayName", row.length > 2 && row[2] != null ? row[2].toString() : "");
                String displayName = row.length > 2 && row[2] != null ? row[2].toString() : (row.length > 1 && row[1] != null ? row[1].toString() : "");
            ...
            Map<String, Object> data = new HashMap<>();
            data.put("plants", list);
            aopMessageVM.setCode(200);
            aopMessageVM.setMessage("Plant dropdown fetched successfully");
            aopMessageVM.setData(data);
            return aopMessageVM;
        } catch (IllegalArgumentException e) {
            throw new RestInvalidArgumentException("Invalid UUID format for Site ID", e);
        } catch (Exception ex) {
            throw new RuntimeException("Failed to fetch plant dropdown for site AOP report", ex);
        }
    }

    @Override
    public byte[] exportMajorSafetyImprovementInitiative(String aopYear, String siteId, boolean isAfterSave, List<MajorSafetyImprovementInitiativeDTO> errorList) {
        try {
            List<MajorSafetyImprovementInitiativeDTO> dtoList;
            if (errorList != null && !errorList.isEmpty()) {
                dtoList = errorList;
            } else {
                AOPMessageVM vm = getMajorSafetyImprovementInitiative(aopYear, siteId);
                if (vm != null && vm.getData() instanceof Map) {
                    @SuppressWarnings("unchecked")
                    Map<String, Object> dMap = (Map<String, Object>) vm.getData();
                    Object listObj = dMap.get("majorSafetyImprovementInitiativeList");
                    if (listObj instanceof List) {
                        @SuppressWarnings("unchecked")
                        List<MajorSafetyImprovementInitiativeDTO> fetchedList = (List<MajorSafetyImprovementInitiativeDTO>) listObj;
                        dtoList = fetchedList;
                    } else {
            ...
            org.apache.poi.ss.usermodel.Sheet sheet = workbook.createSheet("Major Safety Improvement");

            // Header Style: Background color (Grey) + Bold font + All Borders + Centered
            ...
            for (MajorSafetyImprovementInitiativeDTO dto : dtoList) {
                ...
                if (isAfterSave) {
                    ...
                    org.apache.poi.ss.usermodel.Cell errCell = row.createCell(10);
            ...
            sheet.setColumnHidden(8, true); // Hide PlantId
            ...
            throw new RuntimeException("Failed to export Major Safety Improvement Initiative excel", e);
        }
    }

    @Override
    public AOPMessageVM importMajorSafetyImprovementInitiative(String aopYear, String siteId, org.springframework.web.multipart.MultipartFile file) {
        AOPMessageVM aopMessageVM = new AOPMessageVM();
        ...
        try {
            ...
            AOPMessageVM plantDropdownRes = getPlantDropdownForSiteAOPReport(siteId);
            ...
            List<MajorSafetyImprovementInitiativeDTO> allProcessedRows = new ArrayList<>();
            List<MajorSafetyImprovementInitiativeDTO> failedList = new ArrayList<>();
            ...
            try (org.apache.poi.ss.usermodel.Workbook workbook = new org.apache.poi.xssf.usermodel.XSSFWorkbook(file.getInputStream())) {
                ...
                while (rowIterator.hasNext()) {
                    ...
                    String categoryRaw = getStringCellValue(row.getCell(2));
                    String outcomeRaw = getStringCellValue(row.getCell(3));
                    String recommendationRaw = getStringCellValue(row.getCell(4));
                    Date targetDateRaw = parseDateFromCell(row.getCell(5));
                    String responsibilityRaw = getStringCellValue(row.getCell(6));
                    String idRaw = getStringCellValue(row.getCell(7));
                    String plantIdRaw = getStringCellValue(row.getCell(8));

                    // Skip empty rows
                    if ((plantRaw == null || plantRaw.isBlank()) && (descriptionRaw == null || descriptionRaw.isBlank())
                            && (categoryRaw == null || categoryRaw.isBlank()) && (outcomeRaw == null || outcomeRaw.isBlank())
                            && (recommendationRaw == null || recommendationRaw.isBlank())
                            && targetDateRaw == null && (responsibilityRaw == null || responsibilityRaw.isBlank())) {
                        continue;
                    }
                    ...
                    dto.setCategory(categoryRaw);
                    dto.setOutcome(outcomeRaw);
                    dto.setRecommendation(recommendationRaw);
                    dto.setTargetDate(targetDateRaw);
                    dto.setResponsibility(responsibilityRaw);
                    dto.setRemark(responsibilityRaw);
                    ...
                    if (plantRaw == null || plantRaw.trim().isEmpty()) {
                        ...
                        // Basic validation: Plant availability for this site
                    ...
                    if (errors.length() > 0) {
                        ...
                        try {
                            ...
                            if (dto.getId() != null && !dto.getId().isBlank() && !dto.getId().startsWith("temp")) {
                                try {
                                    Optional<MajorSafetyImprovementInitiative> opt = majorSafetyImprovementInitiativeRepository.findById(UUID.fromString(dto.getId().trim()));
                                    if (opt.isPresent()) {
                                        entity = opt.get();
                                    }
                            ...
                            MajorSafetyImprovementInitiative saved = majorSafetyImprovementInitiativeRepository.save(entity);
            ...
            if (!failedList.isEmpty()) {
                byte[] errorExcel = exportMajorSafetyImprovementInitiative(aopYear, siteId, true, allProcessedRows);
            ...
            aopMessageVM.setMessage("Failed to import Major Safety Improvement Initiative Excel: " + e.getMessage());
            return aopMessageVM;
        }
    }
