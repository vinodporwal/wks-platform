...
//    public List<AOPMCCalculatedData> findAllByYearAndPlantFKId(String year,UUID plantId);
	...
	@Query(value = "SELECT TOP (1000) [Id], [Site_FK_Id], [Plant_FK_Id], [Material_FK_Id], [April], [May], [June], [July], [August], [September], [October], [November], [December], [January], [February], [March], [FinancialYear], [Remarks], [CreatedOn], [ModifiedOn], [MCUVersion], [UpdatedBy],[Vertical_FK_Id],[NormParameterDisplayOrder] FROM [dbo].[vwAOPMCValues] WHERE PLANT_FK_ID = :plantId AND FinancialYear = :year order by [NormParameterDisplayOrder]",
	        nativeQuery = true)
