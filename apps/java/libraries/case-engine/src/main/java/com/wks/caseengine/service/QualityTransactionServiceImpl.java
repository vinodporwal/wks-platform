...
public class QualityTransactionServiceImpl implements QualityTransactionService{
	...
	@Transactional(propagation = Propagation.REQUIRES_NEW)
	@Override
	public AOPMessageVM saveQualityData(String year, String plantFKId,
			List<QualityTransactionDTO> qualityTransactionDTOs) {
		try {
			List<QualityTransactionDTO> failedList = new ArrayList<>();
			UUID plantId = UUID.fromString(plantFKId);
...
@Override
	public byte[] exportQualityData(String year, String plantId, boolean isAfterSave, List<QualityTransactionDTO> dtoList) {
	    try {   
	        if (!isAfterSave) {
	        	AOPMessageVM aopMessageVM = getQualityData(plantId,year);
	        	Map<String, Object> innerMap = (Map<String, Object>) aopMessageVM.getData();
...
// phase 2
	...
	public AOPMessageVM importQualityData(String year,UUID plantId,MultipartFile file) {
		try {
			List<QualityTransactionDTO> data = readQualityTransaction(file.getInputStream(), plantId, year);
			 AOPMessageVM aopMessageVM = saveQualityData(year, plantId.toString(),data);
			 List<QualityTransactionDTO> failedList = (List<QualityTransactionDTO>) aopMessageVM.getData();
			
			if (failedList != null && failedList.size() > 0) {
				byte[] fileByteArray = exportQualityData(year, plantId.toString(), true, failedList);
				String base64File = Base64.getEncoder().encodeToString(fileByteArray);
				aopMessageVM.setData(base64File);
				aopMessageVM.setCode(400);
				aopMessageVM.setMessage("Partial data has been saved");
