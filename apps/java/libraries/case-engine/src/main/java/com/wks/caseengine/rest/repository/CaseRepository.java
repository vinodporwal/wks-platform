...
public interface CaseRepository extends JpaRepository<Case, Long> {
	...
	@NativeQuery("SELECT HierarchyNode_PK_ID FROM [case_management].[dbo].[HierarchyNodes] WHERE DisplayNamePath LIKE :assetName AND isDeleted = 0")
	String gethierarchyNodePKID(@Param(value = "assetName") String assetName);

	@NativeQuery(" select * from cases where hierarchy_node_pk_id in (:assetsPKIds)")
