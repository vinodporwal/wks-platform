...
public class ExcelUtilityServiceImpl implements ExcelUtilityService {
 ...
 // ref : generateFlexibleExcel  | seperated method to disable grids
    public byte[] generateFlexibleExcelGridDisable(Map<String, Object> structure, Map<String, List<List<Object>>> data, String verticalSite) {
        try {
                    	...
                    	for (List<Object> rowData : rows) {
                            ...
                            for (int col = 0; col < rowData.size()-1; col++) {
                                ...
                                if (isOptimizerInput || isOptimizerOutput) {
                                    ...
                                    if(isGridDisabled && "pvcDmd".equalsIgnoreCase(verticalSite)) {
                                        cell.setCellStyle(lockedBorderStyle);
                                     }
                                }
 ...
 // Helper function to handle cell value setting (Date/String/N/A)
    ...
    /**
     * Generates the plant-wise reliability Excel with per-cell locking support.
     *
     * When a table in the structure defines a {@code lockedColumn} index, the cell at
     * that column is rendered with a grey locked style for every row whose second-to-last
     * element is the Boolean value {@code false} (i.e. {@code dto.isEditable() == false}).
     * All other cells keep their normal border style.  Rows where {@code isEditable} is
     * {@code true}, or any row coming from the after-save path (where the marker value is
     * a String, not a Boolean), are left unchanged.
     */
    public byte[] generateFlexibleExcelForReliabilityPlantWise(Map<String, Object> structure, Map<String, List<List<Object>>> data) {
        try {
            Workbook workbook = new XSSFWorkbook();
            CellStyle borderStyle = Utility.createBorderedStyle(workbook);
            CellStyle boldStyle = Utility.createBoldStyle(workbook);
            ...
            for (String sheetName : structure.keySet()) {
                ...
                for (Map<String, Object> table : tables) {
                    ...
                    Boolean isRowMergeRequired = (Boolean) table.get(ExcelConstants.IS_ROW_MERGE_REQUIRED);
                    List<Integer> hiddenColumnsList = (List<Integer>) table.get(ExcelConstants.HIDDEN_COLUMNS);
                    Integer lockedColumn = (Integer) table.get("lockedColumn");
                    ...
                    if (rows != null) {
                        for (List<Object> rowData : rows) {
                            // Determine whether this row's "FY Actual" cell (lockedColumn) should be
                            // locked.  The isEditable flag is placed at rowData.size()-2 as a Boolean
                            // by createExcelPlantWise.  String values (e.g. saveStatus in the
                            // after-save path) are intentionally ignored so nothing is greyed there.
                            boolean lockActualCell = false;
                            if (lockedColumn != null && rowData.size() >= 2) {
                                Object marker = rowData.get(rowData.size() - 2);
                                if (marker instanceof Boolean) {
                                    lockActualCell = !(Boolean) marker;
                                }
                            }
                            ...
                            for (int col = 0; col < rowData.size(); col++) {
                                ...
                                if (lockActualCell && col == lockedColumn) {
                                    cell.setCellStyle(lockedBorderStyle);
                                }
                            }
                        }
                    ...
                    if (isColumnMergeRequired) {
                        if (autoMerge != null && autoMerge.get(ExcelConstants.COLUMNS) != null) {
                            for (int colIndex : (List<Integer>) autoMerge.get(ExcelConstants.COLUMNS)) {
                                ...
                                String lastVal = null;
                                for (int r = startDataRow; r < currentRow; r++) {
                                    Row r2 = sheet.getRow(r);
                                    Cell cell = (r2 != null) ? r2.getCell(colIndex) : null;
                                    String val = getCellStringValue(cell);
                                    if (lastVal == null) {
                                        lastVal = val;
                                        mergeStart = r;
                                ...
                                if (currentRow - 1 > mergeStart) {
                                    sheet.addMergedRegion(new CellRangeAddress(mergeStart, currentRow - 1, colIndex, colIndex));
                                }
                            }
                        }
                    }
                    ...
                    if (isRowMergeRequired) {
                        if (autoMerge != null && autoMerge.get(ExcelConstants.ROWS) != null) {
                            for (int rowIndex : (List<Integer>) autoMerge.get(ExcelConstants.ROWS)) {
                                int mergeStart = 0;
                                String lastVal = null;
                                Row r2 = sheet.getRow(startDataRow + rowIndex);
                                if (r2 == null) continue;
                                for (int c = 0; c < r2.getLastCellNum(); c++) {
                                    Cell cell = r2.getCell(c);
                                    String val = getCellStringValue(cell);
                                    if (lastVal == null) {
                                        lastVal = val;
                                        mergeStart = c;
                                        ...
                                        if (c - 1 > mergeStart) {
                                            sheet.addMergedRegion(new CellRangeAddress(startDataRow + rowIndex, startDataRow + rowIndex, mergeStart, c - 1));
                                        }
                                        lastVal = val;
                                        mergeStart = c;
                                    }
                                }
                                if (r2.getLastCellNum() - 1 > mergeStart) {
                                    sheet.addMergedRegion(new CellRangeAddress(startDataRow + rowIndex, startDataRow + rowIndex, mergeStart, r2.getLastCellNum() - 1));
                                }
                            }
                        }
                    }
