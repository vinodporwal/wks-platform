...
@Repository
public interface JMDHeatRateRepository extends JpaRepository<DummyEntity, Long> {

    // query for drop down list
    @Query(value = "select AssetId, AssetName from PowerGenerationAssets WITH(NOLOCK) where CPPPLANT_FK_Id = :cppId and AssetType = :assetType", nativeQuery = true)
    ...
    @Query(value = "select Id, EquipType, CPPUtility, GTLoad, HeatRate, FreeSteamFactor, Remarks from HeatRateLookup WITH(NOLOCK) where AssetId = :assetId order by GTLoad asc", nativeQuery = true)
    ...
    @Query(value = "SELECT s.AssetId, s.AssetName FROM SteamGenerationAssets s WITH(NOLOCK) " +
    ...
    List<HRSGHeatRateProjection> findHrsgHeatRateByAssetId(@Param("assetId") UUID assetId, @Param("financialYear") String financialYear, @Param("previousFinancialYear") String previousFinancialYear);

    @Query(value = "SELECT curr.Id, curr.AssetName as EquipType, curr.UtilityId as CPPUtility, curr.STGLoad, curr.FinalHeatRate as HeatRate, curr.Remarks, prev.FinalHeatRate as PreviousYearHeatRate, curr.FinalHeatRate, curr.OEMHeatRate, curr.SelectedHeatRate FROM CPP_STGHeatRate curr WITH(NOLOCK) LEFT JOIN CPP_STGHeatRate prev WITH(NOLOCK) ON curr.AssetName = prev.AssetName AND curr.STGLoad = prev.STGLoad AND prev.FinancialYear = :previousFinancialYear WHERE curr.AssetName = :assetName AND curr.FinancialYear = :financialYear ORDER BY curr.STGLoad ASC", nativeQuery = true)
    List<STGHeatRateProjection> findStgHeatRateByAssetName(@Param("assetName") String assetName, @Param("financialYear") String financialYear, @Param("previousFinancialYear") String previousFinancialYear);
}
