...
export const PtaConfigurationApiService = {
  ...
  exportExcel,
  importExcel,
  ShutdownRateExportExcel,
  ShutdownRateImportExcel,
}

async function getData(keycloak, PLANT_ID, AOP_YEAR, type) {
  const url = `${Config.CaseEngineUrl}/task/production-configuration?year=${AOP_YEAR}&plantId=${PLANT_ID}`
  const headers = {
...
async function ShutdownRateExportExcel(
  keycloak,
  PLANT_ID,
  AOP_YEAR,
  EXCEL_NAME,
) {
  const url = `${Config.CaseEngineUrl}/task/export-shutdown-rate-manual-entry?year=${encodeURIComponent(AOP_YEAR)}&plantId=${encodeURIComponent(PLANT_ID)}`
  ...
  try {
    ...
    console.error('Error exporting Shutdown Rate Excel:', e)
    return Promise.reject(e)
  }
}
async function ShutdownRateImportExcel(file, keycloak, PLANT_ID, AOP_YEAR) {
  const url = `${Config.CaseEngineUrl}/task/import-shutdown-rate-manual-entry?plantId=${PLANT_ID}&year=${AOP_YEAR}`
  const formData = new FormData()
  formData.append('file', file)
  const headers = {
    Accept: 'application/json',
