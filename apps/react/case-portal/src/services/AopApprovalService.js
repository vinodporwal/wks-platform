...
async function start(keycloak, plantId, year) {
  ...
  const resp = await fetch(url, { method: 'POST', headers: authHeaders(keycloak) })
  return json(keycloak, resp)
}

// Apply a gate decision. decision = 'APPROVED' | 'REVERTED'.
async function act(keycloak, { taskId, plantId, year, gateName, decision, remark, actorRole }) {
  await keycloak.updateToken(30)
  const resp = await fetch(`${base()}/act`, {
    method: 'POST',
    headers: authHeaders(keycloak),
    body: JSON.stringify({ taskId, plantId, year, gateName, decision, remark, actorRole }),
  })
...
async function getStatus(keycloak, plantId, year) {
  ...
  const resp = await fetch(url, { method: 'GET', headers: authHeaders(keycloak) })
  return json(keycloak, resp)
}

// The caller's "My Approvals" inbox across all plants.
async function getMyPending(keycloak) {
  const resp = await fetch(`${base()}/my-pending`, { method: 'GET', headers: authHeaders(keycloak) })
  return json(keycloak, resp)
}
