...
async function getCatChemChemGradeData(keycloak, plantId, aopYear) {
  const url = `${Config.CaseEngineUrl}/task/catalyst-chemicals-chem-grade-data?plantId=${plantId}&year=${aopYear}`
  const headers = {
...
async function saveCatChemChemGradeData(keycloak, plantId, aopYear, payload) {
  const url = `${Config.CaseEngineUrl}/task/save-catalyst-chemicals-chem-grade-data?plantId=${plantId}&year=${aopYear}`
  const headers = {
    Accept: 'application/json',
    'Content-Type': 'application/json',
    Authorization: `Bearer ${keycloak.token}`,
...
async function exportCatChemChemGradeExcel(keycloak, plantId, aopYear, fileName) {
  const url = `${Config.CaseEngineUrl}/task/catalyst-chemicals-chem-grade-data-export?plantId=${plantId}&year=${aopYear}`
  const headers = {
...
async function importCatChemChemGradeExcel(file, keycloak, plantId, aopYear) {
  const url = `${Config.CaseEngineUrl}/task/catalyst-chemicals-chem-grade-data-import?plantId=${plantId}&year=${aopYear}`
