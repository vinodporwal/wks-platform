...
async function handleCalculateNormalOperationNormsPe(
  ...
  const url = `${Config.CaseEngineUrl}/task/calculate-normal-ops-norms?plantId=${PLANT_ID}&siteId=${siteId}&verticalId=${verticalId}&aopYear=${AOP_YEAR}`
  const headers = {
    Accept: 'application/json',
    Authorization: `Bearer ${keycloak.token}`,
  }
...
async function handleCalculateNormalOperationNorms(PLANT_ID, AOP_YEAR, keycloak) {
  const url = `${Config.CaseEngineUrl}/task/calculate-steady-state-norms?year=${AOP_YEAR}&plantId=${PLANT_ID}`
  const headers = {
    Accept: 'application/json',
    Authorization: `Bearer ${keycloak.token}`,
  }
