...
async function saveShutdownNormsExcelImport(
  file,
  keycloak,
  PLANT_ID,
  AOP_YEAR,
  ...
  let url = `${Config.CaseEngineUrl}/task/import-shutdown-consumption-by-material-id?plantId=${PLANT_ID}&year=${AOP_YEAR}`
...
async function getCatChemCalculationData(keycloak, PLANT_ID, AOP_YEAR) {
  const url = `${Config.CaseEngineUrl}/task/catalyst-chemicals-calculation-data?plantId=${PLANT_ID}&year=${AOP_YEAR}`
  const headers = {
    Accept: 'application/json',
    'Content-Type': 'application/json',
    Authorization: `Bearer ${keycloak.token}`,
