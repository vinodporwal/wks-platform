 ...
 */
export const getColumnWidth = (field, headerTitle, filteredItems, items, baseMin = 110, padding = 70) => {
  let maxLen = String(headerTitle || '').length
  const dataset = filteredItems && filteredItems.length > 0 ? filteredItems : items
  ...
  if (Array.isArray(dataset) && dataset.length > 0) {
    dataset.forEach((row) => {
      ...
      else if (field === 'verticalName') val = row.verticalName || row.vertical || ''
      else if (field === 'year') val = String(row.year || '')
      else if (field === 'gateDisplayName') val = row.gateDisplayName || row.gateName || ''
...
export const getApprovalsColumns = (
  ...
  handleOpenAudit,
    ...
    },
    {
      ...
      title: 'Stage',
      minWidth: getColumnWidth('gateDisplayName', 'Stage', filteredItems, items, 160),
      editable: false,
      cell: (props) => {
    ...
    },
    {
      field: 'assignedRole',
      title: 'Current Role',
      minWidth: getColumnWidth('assignedRole', 'Current Role', filteredItems, items, 140),
      editable: false,
      cell: (props) => {
    ...
    },
    {
      ...
      cell: (props) => {
        const row = props.dataItem || {}
        const rowId = row.id || row.taskId || (row.plantId && row.year ? `${row.plantId}_${row.year}` : '')
