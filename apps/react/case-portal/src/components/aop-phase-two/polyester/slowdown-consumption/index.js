...
import { SlowdownConsumptionApiService } from '../../services/polyester/slowdownConsumptionApiService'
...
const SlowdownConsumption = () => {
  ...
  // Fetch slowdown consumption data
  ...
  const saveChanges = useCallback(async () => {
    try {
      ...
      setLoading(true)
      const res = await SlowdownConsumptionApiService.saveSlowdownConsumption(
        keycloak,
        PLANT_ID,
        AOP_YEAR,
        dataToSave,
  ...
  const handleExport = async () => {
    ...
    try {
      ...
      console.error('Error exporting slowdown consumption excel:', error)
      setSnackbarOpen(true)
      setSnackbarData({
  ...
  const handleExcelUpload = async (rawFile) => {
    ...
    try {
      const response =
        await SlowdownConsumptionApiService.importSlowdownConsumption(
          keycloak,
          PLANT_ID,
          AOP_YEAR,
          rawFile,
      ...
      console.error('Error importing slowdown consumption excel:', error)
      setSnackbarOpen(true)
      setSnackbarData({
  ...
  const permissions = {
    ...
    showTitle: true,
    titleName: 'Slowdown Consumption',
  }
