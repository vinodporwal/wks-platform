...
export const validateDateRanges = (
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    // Skip rows without both dates
    if (!row[startDateField] || !row[endDateField]) continue

    const start = new Date(row[startDateField])
    const end = new Date(row[endDateField])
  ...
  if (invalidRows.length > 0) {
    const messages = invalidRows.map(({ row, startDate, endDate }) => {
      const displayName = row[displayFieldName] || `Row ${row.id}`
      return `${displayName}: ${startDate.toLocaleDateString()} - ${endDate.toLocaleDateString()}`
    })

    return `Invalid date range - Start date must be before end date for: ${messages.join('; ')}`
  }

  return ''
}

// Validation function: Check for overlapping date ranges within the same group
export const validateDateOverlaps = (
  rows,
  startDateField = 'startDate',
  endDateField = 'endDate',
  groupByField = 'particulates',
  displayFieldName = 'particulates',
  errorPrefix = 'Overlapping dates found',
) => {
  for (let i = 0; i < rows.length; i++) {
    const row1 = rows[i]

    // Skip rows without required fields
    if (!row1[startDateField] || !row1[endDateField] || !row1[groupByField])
      continue

    const start1 = new Date(row1[startDateField])
    const end1 = new Date(row1[endDateField])

    for (let j = i + 1; j < rows.length; j++) {
      const row2 = rows[j]

      // Skip rows without required fields
      if (!row2[startDateField] || !row2[endDateField] || !row2[groupByField])
        continue

      // Only check overlaps within the same group
      if (row1[groupByField] !== row2[groupByField]) continue

      const start2 = new Date(row2[startDateField])
      const end2 = new Date(row2[endDateField])

      // Check for overlap: start1 <= end2 AND start2 <= end1
      if (start1 <= end2 && start2 <= end1) {
        const displayName = row1[displayFieldName] || row1[groupByField]
        return `${errorPrefix} for ${displayName}: ${start1.toLocaleDateString()} - ${end1.toLocaleDateString()} overlaps with ${start2.toLocaleDateString()} - ${end2.toLocaleDateString()}`
      }
    }
  }
