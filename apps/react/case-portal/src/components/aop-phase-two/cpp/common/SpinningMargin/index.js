...
const SpinningMargin = () => {
  ...
  const EXCEL_NAME = generateExcelName(dataGridStore, 'Spinning_Margin')
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const response = await InputApiService.getMarginData(
        keycloak,
        PLANT_ID_LIST,
        AOP_YEAR,
      )
  ...
  const saveChanges = async () => {
    ...
    try {
      ...
      await InputApiService.saveMarginData(
        keycloak,
        PLANT_ID_LIST,
        AOP_YEAR,
        payload,
  ...
  const handleExcelUpload = async (file) => {
    ...
    setLoading(true)
    try {
      const response = await InputApiService.saveMarginExcel(
        file,
        keycloak,
        PLANT_ID_LIST,
        AOP_YEAR,
      ...
      if (response?.code === 200) {
        ...
        link.href = url
        link.setAttribute('download', `Error File - Margin.xlsx`)
  ...
  const handleExport = async () => {
    ...
    try {
      await InputApiService.exportMarginExcel(
        keycloak,
        PLANT_ID_LIST,
        AOP_YEAR,
        EXCEL_NAME,
