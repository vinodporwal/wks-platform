...
import { NetProductionHoursApiService } from 'components/aop-phase-two/services/common/netProductionHoursApiService'
import { generateExcelName } from 'components/aop-phase-two/common/utilities/excelNameUtil'

const NetProductionHoursGrid = () => {
  ...
  const transformApiData = (apiData) => {
    ...
    return apiData.map((item, index) => {
      const row = {
        id: item.lineId || `row-${index}`,
        particulars: item.Name || '',
      }
      Object.entries(API_MONTH_TO_GRID_MONTH).forEach(([apiKey, gridKey]) => {
        row[gridKey] = item[apiKey] ?? 0
      })
      return row
    })
  }
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const data = transformApiData(rawData)
      setRows(data)
...
export default NetProductionHoursGrid
