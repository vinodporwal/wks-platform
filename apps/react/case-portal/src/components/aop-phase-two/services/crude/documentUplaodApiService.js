...
async function uploadOrUpdateDocument(
  keycloak,
  { transactionId, masterId, verticalId, aopYear, file },
) {
  let url = `${Config.CaseEngineUrl}/task/other-documents?verticalId=${encodeURIComponent(verticalId)}&aopYear=${encodeURIComponent(aopYear)}`
  if (masterId) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      if (resp.status === 500 && errorMsg.startsWith('HTTP error')) {
        errorMsg =
          'Server Error (500): The uploaded file exceeds the maximum allowed server limit (5 MB).'
      }
...
async function deleteDocument(keycloak, transactionId) {
  ...
  try {
    ...
    if (!resp.ok) {
      throw new Error(
        `Failed to delete document: ${resp.status} ${resp.statusText}`,
      )
    }
    return json(keycloak, resp)
  } catch (e) {
...
async function saveOrUpdateOtherDocumentInformation(
  keycloak,
  verticalId,
  aopYear,
  informationList,
