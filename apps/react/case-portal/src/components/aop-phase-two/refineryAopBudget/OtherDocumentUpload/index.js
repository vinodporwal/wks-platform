...
const OtherDocumentUpload = ({ permissions }) => {
    ...
    try {
      ...
      if (response?.code === 200 && Array.isArray(response?.data)) {
        const cleaned = response.data.map((row) => {
          const rawFileName =
            row.fileName || row.file_name || row.originalFileName || ''
          const rawSize =
            row.fileSize ??
            row.size ??
            row.file_size ??
          ...
          return {
            ...
            size:
              typeof row.size === 'number' ? formatBytes(row.size) : row.size,
          }
        })
        setRows(cleaned)
  ...
  const fetchAdditionalInfo = useCallback(async () => {
    ...
    try {
      const infoResp =
        await DocumentUploadApiService.getOtherDocumentInformation(
          keycloak,
          VERTICAL_ID,
          AOP_YEAR,
        ).catch((err) => {
      ...
      if (
        infoResp?.code === 200 &&
        Array.isArray(infoResp?.data) &&
        infoResp.data.length > 0
      ) {
