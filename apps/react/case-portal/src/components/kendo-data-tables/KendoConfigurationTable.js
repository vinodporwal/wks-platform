...
const ConfigurationTable = () => {
  ...
  const { verticalChange, yearChanged, oldYear, plantID, plantObject, siteObject,
    verticalObject, year, } = dataGridStore
  ...
  console.log('PLANT_ID:', PLANT_ID, 'AOP_YEAR:', AOP_YEAR, 'plantObject:', plantObject, 'year:', year);
  const vcmVertical = verticalChange?.selectedVertical?.name?.toLowerCase().trim()
  ...
  const getConfigurationTabsMatrix = async () => {
    setLoading(true)
    try {
      var response = await DataService.getConfigurationTabsMatrix(keycloak,PLANT_ID, AOP_YEAR, SITE_ID, VERTICAL_ID)
  ...
  useEffect(() => {
    ...
    getConfigurationExecutionDetails()
    getAopSummary()
    let vertical = verticalChange?.selectedVertical?.name;
