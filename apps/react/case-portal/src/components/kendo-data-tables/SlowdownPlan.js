...
export default function ShutdownSlowdownPlan() {
  ...
  const fetchData = useCallback(async () => {
    ...
    setLoading(true)
    try {
      const res = await SiteReportDataService.getSlowdownPlan(
        keycloak,
        SITE_ID,
        AOP_YEAR,
      )
  ...
  const saveChanges = React.useCallback(async () => {
    try {
      ...
      // 3. Save to API
      const response = await SiteReportDataService.saveSlowdownPlan(
        keycloak,
        SITE_ID,
        AOP_YEAR,
        payload,
  ...
  const deleteRowData = async (paramsForDelete) => {
    ...
    try {
      ...
      if (idFromApi) {
        await SiteReportDataService.deleteSlowdownPlan(idFromApi, keycloak)
        setRows((prevRows) => prevRows.filter((row) => row.id !== deleteId))
        setSnackbarOpen(true)
        setSnackbarData({
