...
const SelectivityData = (props) => {
  ...
  useEffect(() => {
    const getAllGrades = async () => {
      try {
        const data = await DataService.getAllGrades(keycloak, PLANT_ID, AOP_YEAR)
  ...
  const downloadExcelForConfiguration = async () => {
    ...
    try {
      if (props?.tabIndex != 1) {
        await DataService.getConfigurationExcel(keycloak, gradeId, PLANT_ID, AOP_YEAR)
      } else {
  ...
  const handleLoad = async () => {
    ...
    try {
      await DataService.getConfigurationExcel(keycloak , PLANT_ID, AOP_YEAR)
    } catch (error) {
      console.error('Error!', error)
      setSnackbarOpen(true)
      setSnackbarData({
