...
export default function GroupMaterial({ onSaveSuccess }) {
  ...
  const saveChanges = useCallback(async () => {
    try {
      ...
      const payload = data.map((item) => {
        ...
        return {
          ...
          normParameterTypeFkId:
            item.normParameterTypeFkId || item.normParameterFKId || null,
          normParameterFKId:
            item.normParameterFKId ||
            item.normParameterTypeFkId ||
            item.idFromApi ||
            item.id ||
            null,
      ...
      let response
      if (
        typeof PlantAopReportApiService.saveGroupMaterialDetails === 'function'
      ) {
        response = await PlantAopReportApiService.saveGroupMaterialDetails(
          keycloak,
          AOP_YEAR,
          payload,
        )
      } else {
        response = await PlantAopReportApiService.saveGroupedSelection(
          keycloak,
          payload,
        )
      }
