...
const MaintenanceTable = () => {
  ...
  const { verticalChange, yearChanged, oldYear, plantID, plantObject, siteObject, verticalObject, year } = useSelector(
    (s) => s.dataGridStore,
  )
...
const lowerVertName = verticalChange?.selectedVertical?.toLowerCase()
    ...
    () => ({
      ...
      serviceFn:
        lowerVertName === 'cracker'
          ? (keycloak) =>MaintenanceDetailsApiService.getCrackerMaintenanceData(keycloak, PLANT_ID, AOP_YEAR)
          : (keycloak) =>MaintenanceDetailsApiService.getMaintenanceData(keycloak, PLANT_ID, AOP_YEAR),
  ...
  }, [keycloak, fetchData, PLANT_ID, AOP_YEAR])

  useEffect(() => {
    if (!PLANT_ID || !SITE_ID || !VERTICAL_ID || !AOP_YEAR) return
    fetchData()
  }, [fetchData, oldYear, yearChanged, plantID,AOP_YEAR])
